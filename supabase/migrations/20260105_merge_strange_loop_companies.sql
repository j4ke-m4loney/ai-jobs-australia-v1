-- Merge Strange Loop duplicate companies
-- Keep: "Strange Loop Labs" with ID starting "80ac4fda"
-- Delete: "Strange Loop" with ID starting "950f5fb5"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE '80ac4fda%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '950f5fb5%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "80ac4fda" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "950f5fb5" not found';
    END IF;
