-- Merge Australian Unity duplicate companies
-- Keep: Company with ID starting "431898b2..."
-- Delete: Company with ID starting "3e3ff4dd..."
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE '431898b2%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '3e3ff4dd%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "431898b2..." not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "3e3ff4dd..." not found';
    END IF;
