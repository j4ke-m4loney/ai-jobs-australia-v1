-- Merge XPT Software duplicate companies
-- Keep: "XPT Software" with ID starting "a8f4b6d4"
-- Delete: "XPT Software Australia Pty Ltd" with ID starting "94e90bf6"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'a8f4b6d4%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '94e90bf6%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "a8f4b6d4" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "94e90bf6" not found';
    END IF;
