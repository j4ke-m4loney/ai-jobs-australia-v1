-- Merge AGL duplicate companies
-- Keep: "AGL" with ID starting "c4ffc512"
-- Delete: "AGL Energy" with ID starting "33adf042"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'c4ffc512%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '33adf042%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "c4ffc512" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "33adf042" not found';
    END IF;
