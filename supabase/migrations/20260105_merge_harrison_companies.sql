-- Merge Harrison duplicate companies
-- Keep: "Harrison AI" with ID starting "e9fab7aa"
-- Delete: "Harrison.ai" with ID starting "bd26f15c"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'e9fab7aa%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE 'bd26f15c%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "e9fab7aa" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "bd26f15c" not found';
    END IF;
