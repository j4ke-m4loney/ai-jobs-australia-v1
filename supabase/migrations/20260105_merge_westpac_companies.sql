-- Merge Westpac duplicate companies
-- Keep: "Westpac Group" with ID starting "0d997771"
-- Delete: "Westpac Bank" with ID starting "2d9e6be1"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE '0d997771%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '2d9e6be1%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "0d997771" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "2d9e6be1" not found';
    END IF;
