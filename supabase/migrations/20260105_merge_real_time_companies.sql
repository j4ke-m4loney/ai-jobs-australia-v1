-- Merge Real Time duplicate companies
-- Keep: "Real Time Australia" with ID starting "cb7949bf"
-- Delete: "Real Time" with ID starting "911eff13"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'cb7949bf%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '911eff13%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "cb7949bf" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "911eff13" not found';
    END IF;
