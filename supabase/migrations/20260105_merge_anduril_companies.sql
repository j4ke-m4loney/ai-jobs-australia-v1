-- Merge Anduril duplicate companies
-- Keep: "Anduril" with ID starting "1ce523dd"
-- Delete: "Anduril Industries" with ID starting "5174d616"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE '1ce523dd%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '5174d616%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "1ce523dd" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "5174d616" not found';
    END IF;
