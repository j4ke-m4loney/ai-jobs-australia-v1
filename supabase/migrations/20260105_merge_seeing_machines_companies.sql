-- Merge Seeing Machines duplicate companies
-- Keep: "Seeing Machines" with ID starting "b3ccd715"
-- Delete: "Seeing Machines Limited" with ID starting "5e71cb02"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'b3ccd715%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '5e71cb02%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "b3ccd715" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "5e71cb02" not found';
    END IF;
