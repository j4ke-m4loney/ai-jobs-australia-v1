-- Merge Boston Consulting Group duplicate companies
-- Keep: "Boston Consulting Group" with ID starting "07334810"
-- Delete: "The Boston Consulting Group" with ID starting "0dcb752e"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE '07334810%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '0dcb752e%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "07334810" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "0dcb752e" not found';
    END IF;
