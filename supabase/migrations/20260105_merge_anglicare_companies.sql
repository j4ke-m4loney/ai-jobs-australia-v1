-- Merge Anglicare duplicate companies
-- Keep: "Anglicare" with ID starting "c839a180"
-- Delete: "Anglicare Sydney" with ID starting "5fb933e9"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'c839a180%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE id::text LIKE '5fb933e9%'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "c839a180" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "5fb933e9" not found';
    END IF;
