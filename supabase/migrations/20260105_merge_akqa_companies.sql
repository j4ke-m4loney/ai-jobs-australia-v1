-- Merge AKQA duplicate companies
-- Keep: "AKQA" with ID starting "a7afc659"
-- Delete: "AKQA Media"
...
BEGIN
    ...
    FROM companies
    WHERE id::text LIKE 'a7afc659%'
    LIMIT 1;

    SELECT id, name INTO delete_company_id, delete_company_name
    FROM companies
    WHERE name = 'AKQA Media'
    LIMIT 1;

    -- Check if companies exist
    IF keep_company_id IS NULL THEN
        RAISE EXCEPTION 'Company with ID starting "a7afc659" not found';
    END IF;

    IF delete_company_id IS NULL THEN
        RAISE EXCEPTION 'Company "AKQA Media" not found';
    END IF;
